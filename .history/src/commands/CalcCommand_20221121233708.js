    ...
    if (["+", "-", "*", "/", "=", "C"].indexOf(value) != -1) {
      ...
      if (this.currentDisplayValue == 0 || this.currentOperation == null) {
        ...
        switch (this.currentOperation) {
          case "+":
            this.currentDisplayValue = Utils.add(
              Number(this.currentDisplayValue),
              Number(this.lastDisplayValue)
            );

            break;
          case "-":
            this.currentDisplayValue = Utils.sub(
              Number(this.lastDisplayValue),
              Number(this.currentDisplayValue)
            );
            ...
            break;
          case "*":
            this.currentDisplayValue = Utils.mul(
              Number(this.currentDisplayValue),
              Number(this.lastDisplayValue)
            );
            ...
            break;
          case "/":
            this.currentDisplayValue = Utils.div(
              Number(this.lastDisplayValue),
              Number(this.currentDisplayValue)
            );
